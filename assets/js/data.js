const data = {
    title: 'Senior Full Stack PHP/Javascript Developer',
    introduction: 'Hello! I\'m Youssef, a full stack developer, a PHP enthusiast, and a guy slighty obsessed for code quality.',
    services: [
        { label: 'Back-end', description: 'Utilising php frameworks such as Symfony or creating bespoke code, I\'ve written services supporting thousands of users, including REST APIs, e-learning applications and more.', img: 'assets/img/back-end.svg', alt: 'Back End Developer' },
        { label: 'Front-end', description: 'As a javascript developer, I have experience in HTML5 and CSS3 techniques working with jQuery or more advanced javascript frameworks such as VueJs/ReactJs/Angular', img: 'assets/img/front-end.svg', alt: 'Front End Developer' },
        { label: 'Problem Solving', description: 'As a senior developer, solving problems needs more than writing code and getting results.<br />Design patterns, software architecture, performance, budget, target and understanding the big picture what yield results', img: 'assets/img/consultancy.svg', alt: 'Coding' },
    ],
    certifs: [
        { label: 'Zend Certified PHP Engineer (2017-PHP)', img: '/assets/img/certif/certif-php.gif', href: 'https://www.zend-zce.com/en/yellow-pages/ZEND032239' },
        { label: 'Symfony 5 Certified Developer', img: '/assets/img/certif/certif-symfony.png', href: 'https://connect.symfony.com/profile/moghreb' },
        { label: 'The Professional Scrum Master I (PSM I) ', img: '/assets/img/certif/certif-psmi.svg', href: 'https://www.scrum.org/user/345856' },
        { label: 'GitLab Certified Associate', img: '/assets/img/certif/certif-gitlab.png', href: 'https://badgr.com/public/assertions/ccH_dcC1T7eFD2MuGsu6_Q?identity__email=youssef.benhssaien@gmail.com' },
    ],
    skills : [
        { label: 'PHP', count: 90 },
        { label: 'MySQL', count: 70 },
        { label: 'Javascript', count: 70 },
        { label: 'Git', count: 70 },
        { label: 'HTML/CSS', count: 70 },
        { label: 'Docker', count: 50 },
    ],
    enterprises : [
        { label: 'https://atos.net', img: 'https://atos.net/wp-content/themes/atos/images/atos-logo-menu-bar.png' },
        { label: 'https://www.capgemini.com', img: 'https://www.capgemini.com/wp-content/themes/capgemini-komposite/assets/images/logo.svg' },
        { label: 'https://www.avito.ma', img: 'https://www.avito.ma/img/logo-rbnd.png' },
        { label: 'https://www.cgi.com', img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/32/CGI_logo.svg/1200px-CGI_logo.svg.png' },
        { label: 'https://www.open.global', img: 'https://www.open.global/sites/all/themes/opengroupe_t/assets/img/logo_@2x.png' },
        { label: 'https://www.ausy.fr', img: 'https://www.ausy.fr/sites/all/themes/ausy/logo.png' },
        { label: 'https://www.al-enterprise.com', img: 'https://web-assets.al-enterprise.com/-/media/assets/internet/images/h-to-m/logo.png' },
        { label: 'https://www.laposte.fr', img: 'https://www.laposte.fr/_ui/eboutique/images/fusion/logolp_part.svg' },
        { label: 'https://www.soprasteria.com', img: 'https://www.soprasteria.com/ResourcePackages/Bootstrap4/assets/dist/logos/logo-soprasteria.png' },
        { label: 'https://www.docaposte.com', img: 'https://www.docaposte.com/bundles/docapostcore/img/header/logo-docaposte.svg' },
        { label: 'https://www.groupama.com/fr/fiche/groupama-supports-et-services', img: 'https://media-exp1.licdn.com/dms/image/C4D0BAQEacBVWwr6osw/company-logo_200_200/0/1619076541363?e=1630540800&v=beta&t=cW4xcO3YVdhfeM6dSqFjh2zFMJjSUDtyKb9ZmeHjVYI' },
        { label: 'https://bananacloud.fr', img: 'https://bananacloud.fr/wp-content/uploads/2019/11/Banana_Cloud_digital-marketing_logo_basic-1.svg' },
        { label: 'https://www.fnac.com', img: 'https://media-exp1.licdn.com/dms/image/C4D0BAQFbNeLrzFidQA/company-logo_200_200/0/1519865140136?e=1630540800&v=beta&t=majmEs_cI95r_HVoApKWytkfIAZLDGZIUPQffneZf4U' },
        { label: 'https://www.orange.fr', img: 'https://c.woopic.com/logo-orange.png' },
    ],
    stats: [
        { label: 'Years Experience', count: Math.floor((Date.now() - new Date('03-03-2014'))/1000/60/60/24/365)},
        { label: 'professional Projects', count: 22 },
        { label: 'Personal projects', count: 16 },
        { label: 'Open source projects', count: 3 },
    ],
    contacts: [
        { label: 'E-mail', img: 'assets/img/social/gmail.png', href: 'mailto:youssef.benhssaien@gmail.com' },
        { label: 'Linkedin', img: 'assets/img/social/linkedin.png', href: 'https://www.linkedin.com/in/youssefbenhssaien' },
        { label: 'Github', img: 'assets/img/social/github.png', href: 'https://github.com/ybenhssaien' },
        { label: 'Medium', img: 'assets/img/social/medium.png', href: 'https://youssefbenhssaien.medium.com' },
        { label: 'Stackoverflow', img: 'assets/img/social/stack-overflow.png', href: 'https://stackoverflow.com/users/story/7326498' },
        { label: 'Twitter', img: 'assets/img/social/twitter.png', href: 'https://twitter.com/ysfbenhssaien' },
        { label: 'Dev.to', img: 'assets/img/social/devto.png', href: 'https://dev.to/ybenhssaien' },
        { label: 'Instagram', img: 'assets/img/social/instagram.png',  href: 'https://www.instagram.com/youssef.benhssaien' },
    ],
};